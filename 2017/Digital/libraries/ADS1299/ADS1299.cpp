...
void ADS1299::setup(int _DRDY, int _CS){
    ...
    SPCR |= _BV(MSTR); // Set Arduino in Master mode 
    SPCR |= _BV(SPE); // Set to 1 to enable SPI
    ...
    //set data mode
    SPCR = (SPCR & ~SPI_MODE_MASK) | SPI_DATA_MODE; //clock polarity = 0; clock phase = 1 (pg. 12)
    
    //set bit order
    SPCR &= ~(_BV(DORD)); ////SPI data format is MSB (pg. 25)
    
    // Print SPI Control register
    Serial.println(SPCR)
    ...
    tCLK = 0.000666; //maximum clock period = 666 ns (Datasheet, pg. 12)
    outputCount = 0;
}
...
void ADS1299::WAKEUP() {
    ...
    delay(4.0*tCLK);  //must way at least 4 tCLK cycles before sending another command (Datasheet, pg. 40)
}

//Enters STANDBY Mode
//Once in STANDBY Mode, do not send any other commands other than WAKEUP
...
//Resets registers to default values and resets digital filter cycle
void ADS1299::RESET() {
    digitalWrite(CS, LOW);
    transfer(_RESET);
    //delay(10); //Original code
    delay(18.0*tCLK); //must wait 18 tCLK cycles to execute this command (Datasheet, pg. 41)
    digitalWrite(CS, HIGH);
}

//Starts data conversions
void ADS1299::START() {
    digitalWrite(CS, LOW);
    transfer(_START);
    digitalWrite(CS, HIGH);
...
//Stops data conversions (Conversion in progress is completed before all conversions cease)
void ADS1299::STOP() {
    digitalWrite(CS, LOW);
    transfer(_STOP);
    digitalWrite(CS, HIGH);
...
// Enables Read Data Continuous mode: Conversion data is placed in the output register and can 
// be shifted out on each DRDY (Datasheet, pg.40) without issuing RDATA commands each time
void ADS1299::RDATAC() {
    digitalWrite(CS, LOW);
    transfer(_RDATAC);
    digitalWrite(CS, HIGH);
...
//Stops Read Data Continuous mode. If device is in RDATAC mode, SDATAC must be issued before any
//other commands are sent to the device
void ADS1299::SDATAC() {
    digitalWrite(CS, LOW);
    transfer(_SDATAC);
    digitalWrite(CS, HIGH);
...
// Read single register at _address
void ADS1299::RREG(byte _address) {
    byte opcode1 = _RREG + _address; //001rrrrr; _RREG = 00100000 and _address = rrrrr

    //Prepare for reading
    digitalWrite(CS, LOW); //Low to communicated
    transfer(_SDATAC); //SDATAC must be issued before reading registers if in RDATAC mode
...
// Read several registers at consecutive memory addresses starting at _address
